...
public class SudokuSolver implements ISudokuSolver {
	...
	public void setup(int size1) {
		...
		for(int i = 0; i < size*size*size*size ; i++){ 
			ArrayList<Integer> domain = addDomainValues();
			D.add(i,domain);}
		
	}
	
	public ArrayList<Integer> addDomainValues(){
		ArrayList<Integer> domain = new ArrayList<Integer>();
		for (int i =0; i<size*size; i++){ domain.add(i+1);
		}
		return domain;
	}
	...
	public boolean solve() {
		...
		//INITIAL_FC
		boolean initial = INITIAL_FC(asn);
		
		//FC
		if (initial){ 
			ArrayList<Integer> solved = FC(asn);
			if (asn != null){
			this.puzzle = GetPuzzle(asn);
			return true;
			}
			return false;
		}
...
//	
	public ArrayList<Integer> FC(ArrayList<Integer> asn) {
		// Get the index of the first zero. If there isn't any return a solution. 
		int X = asn.indexOf(0);
		if (X == -1) return asn;
		
		ArrayList<ArrayList<Integer>> Dold = copyDomain(D);		// Copy the Domain of all variables.
		ArrayList<Integer> domainOfX = new ArrayList(D.get(X)); // Copy the specific domain of X
			
		for (int V: domainOfX){
			if (AC_FC(X, V)){
				asn.set(X, V);	// if consistent set the variable value
				
				ArrayList<Integer> R = FC(asn); // And get recursive, until it runs into null
				if (R!= null) return R;
					
				asn.set(X, 0); // if R is null you can't revise anymore and the current value is invalid. Go on. 
				D = copyDomain(Dold); // Return the domain to the old state
				
			}
			else D = copyDomain(Dold); // If the forward checking algorithm is not happy
		}
        
		return null; //in case the alg fails 
	}

	/** Helper function used to copy the big D.
	 * 
	 * @param array
	 * @return
	 */
	private ArrayList<ArrayList<Integer>> copyDomain(ArrayList<ArrayList<Integer>> array){
		ArrayList<ArrayList<Integer>> copy = new ArrayList<ArrayList<Integer>>(array.size());
		for(ArrayList<Integer> domain: array){
		...
		public boolean REVISE(int Xi, int Xj){
			...
			// Gets the domains of Xi and Xj
			
			ArrayList<Integer> Di = D.get(Xi);
			ArrayList<Integer> Dj = D.get(Xj);	
			
			// Go trough all possible values.
			// Thats a bigass for loop.
			/*Goes trough each value Xi. For each value Xi, creates a new list and fills it with zeroes.
			 * Then it sets the Xi-th variable with the value vi 
			 * 
			 * Then go trough all values for Xj and check if it is consistent in the xiEqVal variable. 
			 * 
			 */
			for (int i=0; i<Di.size(); i++){
				...
				// Make a new list and fill it with zero's. 
				// Why? 
				...
				// go trough all values for Xj
				for (int j=0; j<Dj.size(); j++){
					Integer vj = (Integer) Dj.get(j);
					
					//Check if it is consistent with hte corresponding X value
				...
				// Else remove the value Vi from the domain Di of Xi. Return true. 
				if (hasSupport == false) {
